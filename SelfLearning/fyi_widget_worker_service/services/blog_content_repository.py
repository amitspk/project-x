"""
Blog content repository for MongoDB operations.

Handles all database read/write operations for blog content, summaries, and questions.
Business logic name: BlogContentRepository (not StorageService).
"""
...
class BlogContentRepository:
    """Repository for managing blog content, summaries, and questions (business logic name)."""
    ...
    async def _vector_search_atlas(
        ...
        # Batch fetch blog titles for all URLs in a single query (fixes N+1 problem)
        blog_urls = [url for url, _ in search_results[:limit]]
        blogs_dict = await self.get_blogs_by_urls(blog_urls)
